...
def apv_train(split_no):
    ...
    # Split the APV data for train and test
    apv_train, apv_val, apv_y_train, apv_y_val = train_test_split(apv_feats, apv_scores, test_size=0.20, random_state=split_no)

    # Combine LIVE and APV train and test set
    X_train =np.concatenate((apv_train,rand_train_feats),axis=0)
    y_train =np.concatenate((apv_y_train,rand_train_scores),axis=0)
    X_test =  np.concatenate((apv_val,val_feats),axis=0)
    y_test  np.concatenate((apv_y_val,val_scores),axis=0)#np.concatenate((apv_y_val,val_scores),axis=0)
    ...
    for gamma in np.logspace(-8,1,10):
        for C in np.logspace(1,10,10,base=2):
            ...
            kf = KFold(n_splits=5)
            apv_indices = np.arange(len(apv_train))
            apv_split = kf.split(apv_train)
            cv_score = []
            cv_score2 = []
            for cv_split in range(5):
                ...
                # APV set splitting
                apv_cv_train_indices,apv_cv_val_indices= next(apv_split)#train_test_split(apv_indices, test_size=0.20, random_state=cv_split)
                
                apv_cv_train_feats = apv_train[apv_cv_train_indices]
                apv_cv_train_scores= apv_y_train[apv_cv_train_indices]

                apv_cv_val_feats = apv_train[apv_cv_val_indices]
                apv_cv_val_scores = apv_y_train[apv_cv_val_indices]

                # Combine LIVE and APV data
                cv_train_feats = np.concatenate((apv_cv_train_feats,live_cv_train_feats),axis=0)
                cv_train_scores = np.concatenate((apv_cv_train_scores,live_cv_train_scores),axis=0)

                cv_val_feats = np.concatenate((apv_cv_val_feats,live_cv_val_feats),axis=0)
                cv_val_scores = np.concatenate((apv_cv_val_scores,live_cv_val_scores),axis=0)


                # Do a manual grid search for the best parameters
                scaler = preprocessing.MinMaxScaler(feature_range=(-1,1))
                cv_train_feats = scaler.fit_transform(cv_train_feats)
                cv_val_feats = scaler.transform(cv_val_feats)
                clf = svm.SVR(gamma=gamma,C=C)
                

                clf.fit(cv_train_feats,cv_train_scores)
                cv_score.append(clf.score(cv_val_feats,cv_val_scores))
